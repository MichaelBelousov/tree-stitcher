...
export fn transform_ExecQueryResult(
    query_ctx: *bindings.ExecQueryResult,
    transform: chibi.sexp,
    ctx: chibi.sexp
) chibi.sexp {
    ...
    for (query_ctx.matches[0..match_count]) |maybe_match| {
        if (maybe_match) |match| {
            ...
            while (capture_iter.next()) |capture| {
                ...
                defer node_str.free();
                // TODO: just quote the result and skip the evaluation?
                // or even just substitute the code directly for now?
            ...
            var transform_result = chibi._sexp_eval(transform_ctx, transformed_ast, null);
            ...
            // TODO: type check if it's a number
